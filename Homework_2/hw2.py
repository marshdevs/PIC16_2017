...
def mytype(s):
    ...
    integerQuery = '[0-9]{' + str(strLen) + '}'
    intMatch = re.search(integerQuery, inputString)
    if intMatch:
        return "Integer"

    floatQuery = re.match('[0-9]*\.[0-9]*', inputString)
    if floatQuery:
...
def happiness(text):
    ...
    for item in englishWords:
        try:
            REOBJ = re.search('[A-Za-z]*', item)
            word = item[REOBJ.start():REOBJ.end()]
            totalHappiness += happy.happinessDictionary[word]
            numberOfWords += 1
        except KeyError:
            continue
    return float(totalHappiness)/float(numberOfWords)

def main():
    # Test cases for mytype(s)
    # a = '[1, 2, 3, 54, 465, 24453]'
    # print mytype(a)
    # b = '213431243241232'
    # print mytype(b)
    # c = '123532.12435341225'
    # print mytype(c)
    # d = "sd21dsf34"
    # print mytype(d)
